...
Исправления реализации: проверка пароля и инициализация хешера, актуальные права/активность, проверки admin перед мутациями, собственные операции ключей, права автора статьи, сохранение false DTO, migration ledger, SQLite pragmas, удаление встроенных аккаунтов startup. Дополнительно (12.09.2026): дата рождения хранится в базе как TEXT 'YYYY-MM-DD' — прежний формат time.Time не читался обратно и ломал загрузку пользователя (вход проходил, но каждый запрос отвечал 401); события журнала ключей пишутся с автором-администратором, иначе внешний ключ отбивал их и журнал оставался пустым; почта в профиле не затирается, если поле не передано в запросе.
